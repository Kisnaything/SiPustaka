...
import { useRegisterAnggota, type AnggotaFormData } from "@/lib/hooks/useRegisterAnggota";
...
export default function SignupAnggotaPage() {
  ...
  function update<K extends keyof AnggotaFormData>(key: K, value: AnggotaFormData[K]) {
  ...
  function validate(): boolean {
    ...
    (Object.keys(form) as (keyof AnggotaFormData)[]).forEach((key) => {
      if (key === "fotoProfil") return; // opsional
      if (!String(form[key]).trim()) next[key] = "Field ini wajib diisi";
    });
    setErrors(next);
    ...
    <div className="min-h-screen bg-[#FAFAF9] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-bold text-gray-900">Daftar sebagai Anggota</h1>
          <p className="text-[13px] text-gray-400">Isi data diri sesuai identitas kamu</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Konfirmasi Password" error={errors.konfirmasiPassword}>
              <div className="relative">
                <input
          ...
          <div>
            ...
            >
              ...
              <span className="text-[12px] text-center px-4">
                {fotoName ?? "Klik untuk unggah foto profil (JPG, PNG) — Opsional"}
              </span>
            </button>
            <input
              ref={fileRef}
