...
const dummyDenda = [
  ...
  },
  {
    ...
    tanggal: '5 Jun 2026',
    status: 'Ditolak',
    pesanDitolak: 'Bukti transfer tidak terbaca, silakan unggah ulang.',
  },
]
...
export default function DendaPage() {
  ...
  const filtered = dummyDenda.filter((d) =>
    activeTab === 'Semua' || d.status === activeTab
  )

  // Hitung total denda belum lunas
  const totalBelumLunas = dummyDenda
    .filter((d) => d.status === 'Belum Lunas' || d.status === 'Ditolak')
    .reduce((sum, d) => sum + d.jumlahDenda, 0)

  const jumlahBelumLunas = dummyDenda.filter(
  ...
  const handleFileChange = (id: string, file: File | null) => {
    ...
    // Nanti diganti dengan logic upload ke Supabase Storage
    alert(`File "${file.name}" siap diupload untuk denda ID ${id}`)
  }

  return (
    <div style={{ padding: '28px 32px', fontFamily: "'Plus Jakarta Sans', sans-serif", maxWidth: '1100px' }}>
      ...
      {/* Banner warning — hanya tampil kalau ada denda belum lunas */}
      {jumlahBelumLunas > 0 && (
        <div style={{
          ...
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#CA8A04" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
            <path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/>
            <path d="M12 9v4M12 17h.01"/>
          </svg>
          Kamu memiliki {jumlahBelumLunas} denda belum lunas. Total: Rp {totalBelumLunas.toLocaleString('id-ID')}
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {['BUKU', 'HARI TERLAMBAT', 'JUMLAH DENDA', 'TANGGAL', 'STATUS', 'AKSI'].map((h) => (
            <span key={h} style={{ fontSize: '12px', fontWeight: 500, color: '#374151', letterSpacing: '0.05em' }}>
              {h}
            </span>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#6B7280' }}>
            <p style={{ fontSize: '14px', margin: 0 }}>Tidak ada denda dengan status "{activeTab}".</p>
          </div>
        ) : (
          filtered.map((item, i) => {
              ...
              <div key={item.id}>
                <div
                  style={{
                    ...
                    gridTemplateColumns: '2fr 130px 140px 120px 160px 140px',
                    padding: '16px 20px',
                    borderBottom: i < filtered.length - 1 && !item.pesanDitolak ? '1px solid #E5E7EB' : 'none',
                    alignItems: 'center',
                ...
                >
                  ...
                  <div>
                    {perluUpload ? (
                      ...
                      <span style={{ fontSize: '13px', fontWeight: 500, color: '#D4891A', cursor: 'pointer' }}>
                        Detail
                      </span>
                    )}
                  </div>
